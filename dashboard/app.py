...
# Add project root to path so we can import cortex
PROJECT_ROOT = Path(__file__).resolve().parent.parent
if str(PROJECT_ROOT) not in sys.path:
    sys.path.insert(0, str(PROJECT_ROOT))

from cortex.auth import signup, login, get_user_dir

import streamlit as st

st.set_page_config(page_title="Cortex", layout="wide")
...
def get_user_paths():
    """Get all file paths for the current user."""
    if "username" not in st.session_state or not st.session_state.username:
        return None
    user_dir = get_user_dir(st.session_state.username)
    return {
        "base": user_dir,
        "log": user_dir / "runtime_log.json",
        "plan": user_dir / "plan_status.json",
        "rules": user_dir / "cortex.yaml",
...
if not st.session_state.username:
    ...
    with tab_signup:
        ...
        if st.button("Sign up", type="primary", key="signup_btn"):
            ...
            elif signup(signup_user, signup_pass):
...
from cortex.auth import get_user_vault_path

user_vault_path = get_user_vault_path(st.session_state.username)

def load_user_vault():
    if user_vault_path.exists():
        try:
            with open(user_vault_path, "r") as f:
                return json.load(f)
        except Exception:
            return {}
    return {}

def save_user_vault(data):
    import os
    with open(user_vault_path, "w") as f:
        json.dump(data, f, indent=2)
    os.chmod(user_vault_path, 0o600)

user_keys = load_user_vault()
has_anthropic = bool(user_keys.get("ANTHROPIC_API_KEY"))
has_openai = bool(user_keys.get("OPENAI_API_KEY"))
keys_ready = has_anthropic and has_openai

# Show setup if keys missing or settings open
...
def render_dashboard():
    ...
    with left_col:
        ...
        with col_input:
            new_task = st.text_input("Add task", placeholder="e.g. Write unit tests for the auth module", label_visibility="collapsed", key="new_task_input")
        with col_btn:
        ...
        if add_clicked and new_task and new_task.strip():
            import requests
            try:
                requests.post(f"{API_URL}/plan/task", json={"task": new_task.strip()}, timeout=3)
            except Exception:
                plan = load_plan()
                plan["tasks"].append({"task": new_task.strip(), "status": "pending", "result": None})
                plan["total_tasks"] = len(plan["tasks"])
                with open(get_user_paths()["plan"], "w", encoding="utf-8") as f:
        ...
        if total > 0:
            ...
            st.button("Clear all tasks", on_click=lambda: None, key="clear_tasks_btn")
            if st.session_state.get("clear_tasks_btn"):
                import requests
                try:
                    requests.post(f"{API_URL}/plan/reset", timeout=3)
                except Exception:
                    with open(get_user_paths()["plan"], "w", encoding="utf-8") as f:
        ...
        # File upload
        uploaded_file = st.file_uploader("Upload", type=["pdf", "docx", "txt", "csv", "json", "md"], key="file_upload")
        if uploaded_file is not None:
            upload_dir = get_user_paths()["uploads"]
        ...
        if upload_dir.exists():
            ...
            if files:
                st.markdown(f'<div class="cx-meta">Files: {" / ".join(f.name for f in files)}</div>', unsafe_allow_html=True)
    ...
    with right_col:
        ...
        col_rule_input, col_rule_btn = st.columns([4, 1])
        with col_rule_input:
            new_rule = st.text_input("Add rule", placeholder="e.g. Don't commit personal files to git", label_visibility="collapsed", key="new_rule_input")
        with col_rule_btn:
            rule_add_clicked = st.button("Add", type="primary", use_container_width=True, key="add_rule_btn")

        if rule_add_clicked and new_rule and new_rule.strip():
